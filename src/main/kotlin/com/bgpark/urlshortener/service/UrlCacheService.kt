...
class UrlCacheService(
    ...
    fun shortenUrlLocalCache(hash: String, longUrl: String, shortUrl: String): String {
        log.info("shorten url for hash in local cache, hash : $hash")
        return proxy().shortenUrlCache(hash, longUrl, shortUrl)
    }

    @CachePut(cacheNames = [URL_HASH_CACHE_NAME], key = "#hash")
    fun shortenUrlCache(hash: String, longUrl: String, shortUrl: String): String {
        log.info("shorten url for hash in cache, hash : $hash")
